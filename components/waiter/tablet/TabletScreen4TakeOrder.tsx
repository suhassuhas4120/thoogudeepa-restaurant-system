  ...
  return (
    ...
    >
      <div className="flex flex-col flex-1 min-h-[700px] p-5 font-mono select-none">
        ...
        {/* DOUBLE-COLUMN ITEMS GRID OR CLEAN EMPTY STATE */}
        <div className="grid grid-cols-2 gap-3.5 flex-1 overflow-y-auto pr-1">
          {filteredItems.length === 0 ? (
            <div className="col-span-2 py-16 px-6 text-center rounded-2xl border-2 border-dashed border-slate-300 bg-white flex flex-col items-center justify-center gap-3 my-auto shadow-2xs">
              <div className="h-16 w-16 rounded-full bg-orange-50 text-orange-600 flex items-center justify-center border-2 border-orange-200">
                <UtensilsCrossed className="h-8 w-8" />
              </div>
              <h4 className="font-mono text-sm font-black text-slate-900 uppercase tracking-wide">
                No Dishes Available in {selectedCat}
              </h4>
              <p className="text-xs text-slate-500 font-medium max-w-md leading-relaxed">
                {search
                  ? `No items found matching "${search}". Please check the dish name or select another category.`
                  : 'All items in this section are currently sold out or fresh batches are in preparation in the kitchen. Please check back shortly or choose another category.'}
              </p>
              {(selectedCat !== 'ALL' || search) && (
                <button
                  onClick={() => { setSelectedCat('ALL'); setSearch(''); }}
                  className="mt-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-black text-white text-xs font-mono font-bold shadow-xs cursor-pointer transition"
                >
                ...
                >
                  <div className="relative w-full h-24 border-2 border-dashed border-slate-300 bg-slate-50 rounded-lg flex items-center justify-center gap-2 text-slate-600">
                    ...
                    {isSoldOut ? (
                      ...
                      <span className="absolute top-2 right-2 bg-amber-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded flex items-center gap-1">
